...
public class ReleaseGoodImgGridAdapter extends BaseAdapter {

    private LayoutInflater inflater; // 视图容器
    private Context context;
    private int selectedPosition = -1;// 选中的位置
    private boolean shape;
    private List<GoodInfoImage> listImg;
    ...
    public ReleaseGoodImgGridAdapter(Context context, List<GoodInfoImage> listImg) {
    ...
    public  List<GoodInfoImage>  getListImg(){
        return listImg;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position == listImg.size()) {
            ...
            if("0".equals(listImg.get(position).getImg_id())){
                ImageLoaderForLV.getInstance(context).setImageRes(holder.image, "file://"+listImg.get(position).getUrl() + "");
            }else {
                ImageLoaderForLV.getInstance(context).setImageRes(holder.image, listImg.get(position).getUrl() + "");
            }
